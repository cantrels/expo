...
class LinkPreviewNativeView: ExpoView, UIContextMenuInteractionDelegate {
  private var trigger: LinkPreviewNativeTriggerView?
  private var preview: LinkPreviewNativePreviewView?
  private var interaction: UIContextMenuInteraction?
  private var nextScreenId: String?
  private var actions: [LinkPreviewNativeActionView] = []

  private let linkPreviewNativeNavigation: LinkPreviewNativeNavigation = LinkPreviewNativeNavigation()
  ...
  override func mountChildComponentView(_ childComponentView: UIView, index: Int) {
    if let triggerView = childComponentView as? LinkPreviewNativeTriggerView {
      trigger = triggerView
      if let interaction = self.interaction, self.preview != nil {
        trigger?.addInteraction(interaction)
      }
      super.mountChildComponentView(childComponentView, index: index)
    } else if let previewView = childComponentView as? LinkPreviewNativePreviewView {
      preview = previewView
      ...
      print(
        "ExpoRouter: Unknown child component view (\(childComponentView)) mounted to LinkPreviewNativeView"
      )
    }
  }

  override func unmountChildComponentView(_ child: UIView, index: Int) {
    if child is LinkPreviewNativeTriggerView {
      if let interaction = self.interaction {
        trigger?.removeInteraction(interaction)
      }
      trigger = nil
      super.unmountChildComponentView(child, index: index)
    } else if child is LinkPreviewNativePreviewView {
      preview = nil
      if let interaction = self.interaction {
        trigger?.removeInteraction(interaction)
      }
      ...
      print(
        "ExpoRouter: Unknown child component view (\(child)) unmounted from LinkPreviewNativeView")
    }
  }
...
class PreviewViewController: UIViewController {
  private let linkPreviewNativePreview: LinkPreviewNativePreviewView
  init(linkPreviewNativePreview: LinkPreviewNativePreviewView) {
