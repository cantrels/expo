...
function LinkWithPreview({ experimentalPreview, children, ...rest }) {
    ...
    const { preload, updateNextScreenId, nextScreenId } = (0, useScreenPreload_1.useScreenPreload)(hrefWithoutQuery);
    (0, react_1.useEffect)(() => {
    ...
    return (<native_1.LinkPreviewNativeView nextScreenId={nextScreenId} onActionSelected={({ nativeEvent: { id } }) => {
            actionsHandlers[id]?.();
        }} onWillPreviewOpen={() => {
            preload();
...
function LinkMenu({ children }) {
    ...
    return convertChildrenArrayToActions(react_1.default.Children.toArray(children)).map((action) => {
        return <native_1.LinkPreviewNativeActionView key={action.id} title={action.title} id={action.id}/>;
    });
}
