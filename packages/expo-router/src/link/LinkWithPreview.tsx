...
export function LinkWithPreview({ experimentalPreview, children, ...rest }: LinkProps) {
  ...
  const { preload, updateNextScreenId, nextScreenId } = useScreenPreload(hrefWithoutQuery);
...
export function LinkMenu({ children }: LinkMenuProps) {
  ...
  return convertChildrenArrayToActions(React.Children.toArray(children)).map((action) => {
    return <LinkPreviewNativeActionView key={action.id} title={action.title} id={action.id} />;
  });
}
...
function convertChildrenArrayToActions(children: ReturnType<typeof React.Children.toArray>) {
  return children
    .filter(
      (item): item is ReactElement<LinkMenuItemProps> =>
        isValidElement(item) && item.type === LinkMenuItem
    )
    .map((child, index) => ({
